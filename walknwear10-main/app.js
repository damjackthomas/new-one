...
const useBarcodeBtn = document.getElementById("use-barcode");
...
let codeReader = null;
let currentScanMode = 'add'; // 'add' or 'search'
...
// Browser and Camera Compatibility Check
function checkCameraCompatibility() {
  ...
  console.log('Camera compatibility check:', {
    isSecure,
    hasGetUserMedia,
    isHTTPS,
    isLocalhost,
  ...
  if (!hasGetUserMedia) {
    showScannerError('Your browser does not support camera access. Please use a modern browser like Chrome, Firefox, or Safari.');
    return false;
  }
  
  if (!isSecure && !isLocalhost) {
    showScannerError('Camera access requires HTTPS. Please run this on a secure server (HTTPS) or from localhost.');
    return false;
  }
...
function initializeBarcodeScanner() {
  // Wait for ZXing library to be available
  if (typeof ZXing === 'undefined') {
    console.log('ZXing library not loaded yet, retrying in 1 second...');
    setTimeout(initializeBarcodeScanner, 1000);
    return;
  }
  
  // Check camera compatibility first
  if (!checkCameraCompatibility()) {
    // Disable scanner buttons if not compatible
    if (scanBarcodeBtn) scanBarcodeBtn.disabled = true;
    if (searchBarcodeBtn) searchBarcodeBtn.disabled = true;
    return;
  }
  ...
  try {
    ...
    if (searchBarcodeBtn) searchBarcodeBtn.disabled = false;
  } catch (error) {
    console.error('Error initializing barcode scanner:', error);
    showScannerError('Failed to initialize barcode scanner. Please refresh the page.');
    
    // Disable scanner buttons on error
    if (scanBarcodeBtn) scanBarcodeBtn.disabled = true;
    if (searchBarcodeBtn) searchBarcodeBtn.disabled = true;
  }
}
...
function showCameraSetupHelp() {
  const helpMessage = `
Camera Access Setup Help:

1. For Local Development:
   - Use: http://localhost:5500/new%20one/walknwear10-main/index.html
   - OR: http://127.0.0.1:5500/new%20one/walknwear10-main/index.html
...
3. Browser Permissions:
   ...
   - If denied, go to browser settings > Privacy > Camera > Allow this site

4. Troubleshooting:
   - Make sure no other app is using the camera
   - Refresh the page and try again
   - Use Chrome, Firefox, or Safari (modern browsers)
  `;
...
async function startBarcodeScanner() {
  if (!codeReader) {
    ...
    if (!codeReader) {
      showScannerError('Barcode scanner not initialized. Please refresh the page.');
      return;
    }
  }
  ...
  try {
    ...
    if (typeof ZXing === 'undefined') {
      showScannerError('ZXing library not loaded. Please refresh the page.');
      return;
    }

    // Check if we're on a secure context (HTTPS or localhost)
    if (!isSecureContext()) {
      showScannerError('Camera access requires HTTPS or localhost. Please use a secure server or run from localhost.');
      return;
    }

    // Request camera permission directly first
    try {
      ...
      if (permissionError.name === 'NotAllowedError') {
        ...
        showScannerError('No camera found. Please ensure you have a camera connected.');
      } else {
        showScannerError('Camera access error: ' + permissionError.message);
      }
      return;
    }
    ...
    if (videoInputDevices.length === 0) {
      showScannerError('No camera found. Please ensure you have a camera connected and grant camera permissions.');
      return;
    }
    ...
    if (error.name === 'NotAllowedError') {
      showScannerError('Camera permission denied. Please allow camera access and try again.');
    } else if (error.name === 'NotReadableError') {
      showScannerError('Camera is already in use by another application. Please close other apps using the camera.');
    } else {
      showScannerError('Failed to start camera: ' + error.message + '. Please check camera permissions and try again.');
    }
  }
}
...
function handleBarcodeScan(barcodeText) {
  lastScannedBarcode = barcodeText;
  scannedBarcodeSpan.textContent = barcodeText;
  scannerResult.style.display = 'block';
  stopBarcodeScanner();
  
  // Auto-use barcode after 2 seconds
  setTimeout(() => {
...
function useScannedBarcode() {
  if (currentScanMode === 'add') {
    // Add barcode to SKU field for new product
    productSkuInput.value = lastScannedBarcode;
    productSkuInput.focus();
  } else if (currentScanMode === 'search') {
    // Search for product with this barcode
    searchInput.value = lastScannedBarcode;
...
function openBarcodeScanner(mode = 'add') {
  console.log('Opening barcode scanner with mode:', mode);
  console.log('Modal element:', barcodeScannerModal);
  ...
  const modalTitle = barcodeScannerModal.querySelector('h3');
  if (modalTitle) {
    modalTitle.textContent = mode === 'add' ? 'Scan Barcode for New Product' : 'Scan Barcode to Search';
  }
...
console.log('Setting up barcode scanner event listeners...');
console.log('Scan button:', scanBarcodeBtn);
console.log('Search button:', searchBarcodeBtn);
console.log('Scanner modal:', barcodeScannerModal);

if (scanBarcodeBtn) {
  scanBarcodeBtn.addEventListener('click', (e) => {
    console.log('Scan barcode button clicked!');
    e.preventDefault();
    openBarcodeScanner('add');
  });
  console.log('Scan barcode button listener added');
} else {
  console.error('Scan barcode button not found!');
}

if (searchBarcodeBtn) {
  searchBarcodeBtn.addEventListener('click', (e) => {
    console.log('Search barcode button clicked!');
    e.preventDefault();
    openBarcodeScanner('search');
  });
  console.log('Search barcode button listener added');
} else {
  console.error('Search barcode button not found!');
}
...
if (cameraHelpBtn) {
  cameraHelpBtn.addEventListener('click', showCameraSetupHelp);
}
if (startScannerBtn) {
...
if (useBarcodeBtn) {
  useBarcodeBtn.addEventListener('click', useScannedBarcode);
}
if (scanAgainBtn) {
...
function setupBarcodeScannerEventListeners() {
  console.log('Setting up barcode scanner event listeners...');
  
  // Re-check elements in case they weren't loaded during initial setup
  ...
  console.log('Scanner modal:', modal);
...
async function initApp() {
  ...
  setupBarcodeScannerEventListeners();
  
  // Initialize scanner buttons as disabled until library loads
  if (scanBarcodeBtn) scanBarcodeBtn.disabled = true;
  if (searchBarcodeBtn) searchBarcodeBtn.disabled = true;
